...
import { appelsAPI } from '../api/appels';
...
export default function Home() {
  ...
  useEffect(() => {
    demandesAPI.getMesDemandes()
      .then((res) => {
        if (res.data) {
          setStats(prev => ({ ...prev, dossiersTraites: res.data.length || 0 }));
        }
      })
      .catch(() => {});

    appelsAPI.getActifsEpingles()
      .then((res) => {
        setAnnonces(res.data.results || res.data || []);
      })
              ...
              {annonces.map((a, i) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#666', marginBottom: 8, marginTop: 4, fontWeight: 700, textTransform: 'uppercase' }}>
                    📅 {new Date(a.date_lancement).toLocaleDateString('fr-FR')}
                  </div>
          ...
          {selectedAnnonce && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ fontSize: 13, color: 'var(--muted)' }}>
                Lancé le {new Date(selectedAnnonce.date_lancement).toLocaleDateString('fr-FR')}
              </div>
