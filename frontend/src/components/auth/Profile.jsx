        ...
        <Card className="md:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-ink mt-0.5">{user?.telephone || '77 000 00 00'}</p>
            </div>
            <div>
              <p className="font-mono text-xs text-muted uppercase">Adresse physique</p>
              <p className="font-semibold text-ink mt-0.5">{user?.adresse || 'Campus CROUS-T, Thiès'}</p>
            </div>
            <div>
              <p className="font-mono text-xs text-muted uppercase">Qualité / Statut</p>
              <p className="font-semibold text-ink mt-0.5">
                {user?.est_etudiant ? '🎓 Étudiant de l\'UIDT' : (user?.role === 'OCCUPANT' ? '🔑 Occupant Titulaire' : '🏢 Personnel / Usager')}
              </p>
            </div>
            {user?.est_etudiant && (
              <div>
                <p className="font-mono text-xs text-muted uppercase">Vérification Carte Étudiante</p>
                <StatusBadge statut={user?.statut_verification_etudiant || 'VALIDE'} className="mt-1" />
              </div>
            )}
          </div>
        </Card>
